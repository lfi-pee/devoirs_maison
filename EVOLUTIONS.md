...
| ✅ Corrigé | **Le juge et le jugé lisent désormais le même chiffre.** `validation_non_inscription.py` reconstruisait `maj` de son côté ; trois écarts de plomberie s'y étaient logés — pondération PLM (1 631 personnes), arrondi commune par commune (182), communes absentes du carnet (404). **2 217 personnes** : de quoi faire passer un défaut de reconstruction pour un défaut de l'estimateur, dans un test dont c'est précisément le métier de distinguer les deux. `prep_bake.admin_communes` et `prep_bake.maj_potentielle` sont maintenant la source **unique** ; le test les appelle au lieu de les réécrire, et l'écart tombe à zéro par construction. |
| ⚠️ Limite mesurée | **Ce correctif ne rachète pas le chiffre, et le test le dit.** Au national il ne vaut rien — 49,10 M de majeur·es français·es contre 49,01 M, **84 000 personnes, 0,17 %**. Sa valeur est **géographique** : en **Seine-Saint-Denis**, département le plus étranger de métropole et jusqu'ici pire écart du pays, l'erreur tombe de **+8,6 à +6,9 points**. C'est de loin le plus gros des quatre grands écarts déplacés — le Val-de-Marne recule de 0,6 point (+7,4 → +6,8), les Ardennes et les Hautes-Alpes de 0,2 seulement : ces trois-là ne viennent pas de la nationalité. Le tableau de bord bouge à peine (Spearman 0,480 → 0,477, dispersion des résidus 3,15 → 3,05 points) : on garde le correctif parce que c'est la bonne grandeur, pas parce qu'il résout quoi que ce soit. |
