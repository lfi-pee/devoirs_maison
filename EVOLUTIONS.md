...
| Statut | Détail |
| ------ | ------ |
| ✅ Fait | **Un bandeau de repères en tête de chaque fiche** (`reperes`, [03_panel_info.js](assets/js/03_panel_info.js)) : **habitant·es**, **inscrit·es** (avec un « i » qui dit de quel registre), **majeur·es de nationalité française recensé·es** et le **solde d'inscription**. Montpellier ouvre désormais sur « 302 454 habitant·es · 169 505 inscrit·es · 212 795 majeur·es français·es · 43 290 à inscrire » au lieu d'un titre nu. |
...
## Le réservoir d'inscription n'avait jamais été confronté à une mesure : il en manque 20 %

> Constat : `resinsc` est publié commune par commune depuis le correctif de Montpellier, et
> présenté comme le **levier n°1 du plan d'action**. Personne ne pouvait dire s'il était
> juste. La non-inscription n'est publiée à aucune échelle infra-départementale, donc chaque
> choix de l'estimateur — la part de nationalité, les tranches d'âge, le traitement des
> soldes négatifs — était **infalsifiable** : on ne pouvait qu'argumenter, jamais mesurer.
> Il existe pourtant deux contraintes qui suffisent à le mettre en défaut, et elles disent
> qu'il manque **0,58 M de personnes sur 2,84 M attendues**.

| Statut | Détail |
| ------ | ------ |
| ✅ Fait | **Le solde a maintenant un juge : `validation_non_inscription.py`.** Deux contraintes, qu'aucun réglage interne ne peut satisfaire par construction. **(1) L'identité nationale** : sommée sur toutes les communes, la mal-inscription s'annule exactement — chaque mal-inscrit·e est retranché·e là où il ou elle réside et ajouté·e là où il ou elle est inscrit·e — donc le solde national **doit** valoir la non-inscription pure. L'INSEE la chiffre à 2,9 M, soit **5,8 %** des Français·es majeur·es ([Insee Première n°1986](https://www.insee.fr/fr/statistiques/7766966), présidentielle 2022) ; c'est ce taux qui sert de cible, appliqué à notre propre effectif de majeur·es (49,0 M) : **2,84 M**, chiffre dérivé et non publié. **(2) La forme départementale** : la figure 4 de la même étude publie, pour 100 départements **de résidence** (le titre de la feuille tranche le sens de la ventilation), la part des personnes non inscrites dans leur commune de résidence ; un département qui exporte ses inscriptions doit afficher un solde plus positif. Le test porte sur les inscrit·es de la **présidentielle 2022**, scrutin de référence de l'étude, quand le site publie sur le registre 2024 : on compare ce qui est comparable, pas ce qui est affiché. |
| ✅ Vérifié | **Le signal est réel.** Spearman **0,477** entre le solde départemental et la mesure INSEE en métropole, sur 96 départements (t = 5,27 à 94 degrés de liberté, p ≈ 9 × 10⁻⁷) : le solde dit bien quelque chose du terrain, ce qui n'allait pas de soi pour une soustraction entre deux sources qui ne se parlent pas. |
| ⚠️ Limite mesurée | **Et il est faux d'un décalage, toujours dans le même sens.** Total national **2,26 M contre 2,84 M** : **−20 %**. La droite d'ajustement coupe l'axe à **−0,064** là où la comptabilité l'attend vers **+0,058** — 12 points d'écart, sous tous les départements à la fois. C'est une seule et même chose : la **contamination des listes**, des inscrit·es qui ne sont dans la population résidente de personne (Français·es de l'étranger sur liste communale, radiations en retard). Elle n'est pas modélisée, et c'est elle, pas la nationalité ni les tranches d'âge, qui porte l'essentiel de l'erreur. |
| ✅ Corrigé | **La part de nationalité française est enfin mesurée sur les majeur·es.** `part_fr` était calculée sur **toute** la population, mineur·es compris, et servait pourtant à ramener `pop18` au corps électoral potentiel — un biais que EVOLUTIONS.md documentait comme « de l'ordre du point, non mesurable, compensé ». Il l'est maintenant : [NAT1](https://www.insee.fr/fr/statistiques/8202752) croise nationalité et âge à la commune (et à l'arrondissement PLM), seule table INSEE à le faire sous le département. `part_fr18` pondère la part de Français·es par les tranches adultes ; `part_fr` ne sert plus que de repli, sur les **13 communes** absentes de NAT1. Six autres n'ont de valeur ni d'un côté ni de l'autre — les villages de la Meuse détruits en 1914-1918, sans habitant·e à recenser : elles ne sont pas servies. |
| ✅ Corrigé | **Une tranche muette ne rend plus la commune muette.** Premier jet : une seule des trois tranches adultes absente de NAT1 suffisait à faire retomber toute la commune sur `part_fr` — **234 communes**, alors que **13 seulement** manquaient vraiment à la table. Deux confusions, parce que NAT1 se tait de deux façons qu'il ne faut pas confondre : une tranche **peuplée sans aucun·e Français·e** n'a pas de ligne `INATC = 1`, et son rapport vaut **zéro**, pas « inconnu » (14 communes) ; une tranche **absente ou vide** ne dit rien, et doit sortir du numérateur **et** du dénominateur au lieu d'empoisonner la moyenne (207 communes). Les deux garde-fous posés, **215 communes** rentrent dans le champ et il n'en reste **19** — les 13 absentes, plus 6 villages de la Meuse détruits en 1914-1918 que la table déclare sans un seul adulte. Aucune commune déjà couverte ne bouge d'un centième, et la ligne FRANCE reste à 92,42 %. |
| ✅ Corrigé | **Le juge et le jugé lisent désormais le même chiffre.** `validation_non_inscription.py` reconstruisait `maj` de son côté, et deux écarts de plomberie s'y étaient logés : la **pondération PLM** (`_agreger_plm` repondère la part de nationalité par la population des arrondissements, le test sommait les produits arrondissement par arrondissement — 1 631 personnes) et l'**arrondi commune par commune** (183). **1 814 personnes** : de quoi faire passer un défaut de reconstruction pour un défaut de l'estimateur, dans un test dont c'est précisément le métier de distinguer les deux. `prep_bake.admin_communes` et `prep_bake.maj_potentielle` sont maintenant la source **unique** ; le test les appelle au lieu de les réécrire, et l'écart tombe à **zéro par construction**. |
| ⚠️ Non vérifié | **13 communes sont mesurées mais pas publiées.** Elles ont une ligne recensement et des inscrit·es en 2022, aucune entrée dans le carnet baké — et ce sont les mêmes 13 que NAT1 ne couvre pas. Le test les compte (**+404** sur le solde national), le site ne les affiche pas. L'écart est négligeable ; ce qu'il signale ne l'est peut-être pas, puisqu'il faudrait comprendre pourquoi ces communes sortent du référentiel servi. Défaut de publication, pas de mesure. |
| ⚠️ Limite mesurée | **Ce correctif ne rachète pas le chiffre, et le test le dit.** Au national il ne vaut rien — 49,10 M de majeur·es français·es contre 49,01 M, **84 000 personnes, 0,17 %**. Sa valeur est **géographique** : en **Seine-Saint-Denis**, département le plus étranger de métropole et jusqu'ici pire écart du pays, l'erreur tombe de **+8,6 à +6,9 points**. C'est de loin le plus gros des quatre grands écarts déplacés — le Val-de-Marne recule de 0,64 point (+7,4 → +6,8), les Ardennes de 0,15 et les Hautes-Alpes de 0,20 seulement : ces trois-là ne viennent pas de la nationalité. Le tableau de bord bouge à peine (Spearman 0,480 → 0,477, dispersion des résidus 3,15 → 3,05 points) : on garde le correctif parce que c'est la bonne grandeur, pas parce qu'il résout quoi que ce soit. |
| ⚠️ Non vérifié | **Les Ardennes, +7,4 points, sans explication.** C'est le département **le moins mal-inscrit de France** (10,8 %) et il affiche un solde de 8,0 % — il est désormais notre plus gros écart, devant la Seine-Saint-Denis. Ni la nationalité, ni l'âge, ni la mobilité n'en rendent compte. Tant que ce n'est pas compris, tout modèle de contamination ajusté par-dessus incorporerait l'anomalie au lieu de la corriger. |
| ⚠️ Limite assumée | **L'outre-mer casse la relation et sort de l'ajustement.** Guadeloupe : **23,6 %** côté INSEE, **−10,8 %** côté solde. Ses listes portent une diaspora sans équivalent métropolitain. Les DOM sont mesurés et affichés, mais l'ajustement porte sur la métropole seule — les inclure fait tomber le Spearman de 0,48 à 0,39. Mayotte est hors champ de l'étude INSEE, d'où **100 départements** appariés et non 101. |
